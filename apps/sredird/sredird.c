...
void Usage(void)
{
    ...
    puts("Usage: sredird [-i] <loglevel> <device> <lockfile> <port> [pollingterval]");
...
int main(int argc, char * argv[])
  {
    ...
    /* Disable echoing */
//    PortSettings.c_lflag = (PortSettings.c_lflag & ~ECHO);

    /* Write the port settings to device */
    tcsetattr(DeviceFd,TCSANOW,&PortSettings);

    /* Reset the device fd to blocking mode */
    if (fcntl(DeviceFd,F_SETFL,(fcntl(DeviceFd,F_GETFL) & ~(O_NDELAY)) | O_NONBLOCK) == OpenError)
            ...
            if (FD_ISSET(connfd, &InFdSet))
              {
                ...
                setsockopt(sockfd,SOL_IP,IP_TOS,&SockParm,sizeof(SockParm));
                /* setsockopt(sockfd,SOL_SOCKET,SO_OOBINLINE,&SockParmEnable,sizeof(SockParmEnable)); */

                /* Make reads/writes unblocking */
                ioctl(sockfd,FIONBIO,&SockParmEnable);
