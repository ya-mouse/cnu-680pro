...
// choish_shared_lib_porting
	 ...
	 */
	// choish_shared_lib_porting, for MMAP_FIX
#ifndef CONFIG_MMAP_FIX
    /* Now, shared write to file is supported */
...
// choish_shared_lib_porting 
			...
			if(!error) {
				...
				tblock->rblock->kblock = (void *)vma.vm_start;
//				tblock->rblock->size = len;
				tblock->rblock->size = 0; /* do not count mmapped size */
				tblock->rblock->is_mmapped = 1;
				tblock->rblock->f_op = file->f_op;
				tblock->rblock->mmapped_inode = file->f_dentry->d_inode;
...
#ifdef DEBUG
	printk("kmalloc mmap. pid=%d ddr=0x%x, length=0x%x\n", (unsigned int)(current->pid), result, len);
#endif
	if (!result) {
...
// choish_shared_lib_porting
#ifdef CONFIG_MMAP_FIX
	tblock->rblock->is_mmapped = 0;
	tblock->rblock->f_op = NULL;
	tblock->rblock->mmapped_inode = NULL;
#endif	
...
int do_munmap(struct mm_struct * mm, unsigned long addr, size_t len)
{
	...
	if (!tblock) {
//		remark for unnecessary message print  of boa process
...
// choish_shared_lib_porting
#ifdef CONFIG_MMAP_FIX
			if(tblock->rblock->is_mmapped == 1) {
				struct file_operations *f_op;
				struct inode *inode;
				f_op = tblock->rblock->f_op;
				inode = tblock->rblock->mmapped_inode;
				if(f_op && f_op->munmap) {
					f_op->munmap(inode);
				}
				goto released;
...
// choish_shared_lib_porting
	...
	while((tmp = mm->tblock.next)) {
		freed = 0; // choish_shared_lib_porting
		if (tmp->rblock) {
			if (!--tmp->rblock->refcount) {
// choish_shared_lib_porting
#ifdef CONFIG_MMAP_FIX
				if(tmp->rblock->is_mmapped == 1) {
					struct file_operations *f_op;
					struct inode *inode;
					f_op = tmp->rblock->f_op;
					inode = tmp->rblock->mmapped_inode;
					if(f_op && f_op->munmap) {
						f_op->munmap(inode);
					}
					goto released;
				...
				if (tmp->rblock->kblock) {
					...
					freed += ksize(tmp->rblock->kblock); // choish_shared_lib_porting
...
released:
				...
				freed += ksize(tmp->rblock); // choish_shared_lib_porting
				askedalloc -= sizeof(struct mm_rblock_struct);
				kfree(tmp->rblock);
			}
			tmp->rblock = 0;
		...
		// choish_shared_lib_porting
		freed += ksize(tmp);
		//printk("tblock=0x%x, freed=%d bytes\n", tmp, freed);
		total_freed += freed;
		//choish_end
		askedalloc -= sizeof(struct mm_tblock_struct);
		kfree(tmp);
