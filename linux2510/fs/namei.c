...
int open_namei(const char * pathname, int flag, int mode, struct nameidata *nd)
{
	...
	if (!(flag & O_CREAT)) {
		...
		if (error){
#if 0  // 8MSol_jungmin (03.12.03)
			printk("path lookup error (%s)\n", pathname);
#endif // 8MSol_jungmin_end (03.12.03)
			return error;
		}
		dentry = nd->dentry;
		goto ok;
