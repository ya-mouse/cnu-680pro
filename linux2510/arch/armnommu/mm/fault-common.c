...
void show_pte(struct mm_struct *mm, unsigned long addr)
{
	...
	printk("\n");
#else
	// choish_shared_lib_porting, for degugging
	printk("process(%s) %s(%d): start_code=0x%x, start_stack=0x%x)\n",
		current->comm, __FILE__,__LINE__, mm->start_code, mm->start_stack);
#endif
}
