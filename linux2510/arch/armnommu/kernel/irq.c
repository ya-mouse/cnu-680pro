...
// Radicalis_hans_begin (03.12.11) - to avoid interrupt locking until there is good way
#if 0
			printk(KERN_ERR "IRQ LOCK: IRQ%d is locking the system, disabled\n", irq);
			disable_irq(irq);
#else
			printk(KERN_ERR "IRQ LOCK: IRQ%d is locking the system\n", irq);
#endif
// Radicalis_hans_end (03.12.11)
		}
	} else {
		desc->lck_cnt = 0;
