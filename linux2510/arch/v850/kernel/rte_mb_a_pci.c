...
#define CONFIG_DMCFGA(dev, offs)					      \
   (0x80000000								      \
    | ((offs) & ~0x3)							      \
    | ((dev)->devfn << 8)						      \
    | ((dev)->bus->number << 16))

#define DEF_MB_PCI_READ_CONFIG_OP(pci_type, c_type)			      \
static int								      \
mb_pci_read_config_ ## pci_type (struct pci_dev *dev, int offs, c_type *rval) \
{									      \
	int flags;							      \
	save_flags_cli (flags);						      \
									      \
	MB_A_PCI_PCICR = 0x7;						      \
	MB_A_PCI_DMCFGA = CONFIG_DMCFGA (dev, offs);			      \
									      \
	*rval = *(volatile c_type *)(MB_A_PCI_IO_ADDR + (offs & 0x3));	      \
									      \
...
#define DEF_MB_PCI_WRITE_CONFIG_OP(pci_type, c_type)			      \
static int								      \
mb_pci_write_config_ ## pci_type (struct pci_dev *dev, int offs, c_type val)  \
{									      \
	int flags;							      \
	save_flags_cli (flags);						      \
									      \
	...
	MB_A_PCI_DMCFGA = CONFIG_DMCFGA (dev, offs);			      \
									      \
	*(volatile c_type *)(MB_A_PCI_IO_ADDR + (offs & 0x3)) = val;	      \
									      \
        if (MB_A_PCI_PCISR & 0x2000)					      \
		MB_A_PCI_PCISR = 0x2000;				      \
									      \
...
DEF_MB_PCI_READ_CONFIG_OP (byte, u8);
DEF_MB_PCI_WRITE_CONFIG_OP (byte, u8);
DEF_MB_PCI_READ_CONFIG_OP (word, u16);
DEF_MB_PCI_WRITE_CONFIG_OP (word, u16);
DEF_MB_PCI_READ_CONFIG_OP (dword, u32);
DEF_MB_PCI_WRITE_CONFIG_OP (dword, u32);

static struct pci_ops mb_pci_config_ops = {
	mb_pci_read_config_byte,
	mb_pci_read_config_word,
	mb_pci_read_config_dword,
	mb_pci_write_config_byte,
	mb_pci_write_config_word,
	mb_pci_write_config_dword
};
