/* $Id: traps.c,v 1.1.1.1 2003/11/17 02:33:22 jipark Exp $
 *
...
int is_dsp_inst(struct pt_regs *regs)
{
	...
	/* Safe guard if DSP mode is already enabled... */
	if (regs->sr & 0x00001000)
		return(0);
...
static void
show_stack(unsigned long *start, unsigned long *end)
{
	unsigned long *p;
...
void show_trace_task(struct task_struct *tsk)
{
	...
	if ((sp ^ (unsigned long)tsk) & (PAGE_MASK<<1))
		return;
	show_stack((unsigned long *)sp, ((unsigned long) tsk) + (PAGE_SIZE<<1));
}
