/* $Id: cache-sh4.c,v 1.1.1.1 2003/11/17 02:33:22 jipark Exp $
 *
 *  linux/arch/sh/mm/cache.c
 *
 * Copyright (C) 1999, 2000  Niibe Yutaka
 * Copyright (C) 2003  David McCullough (7751R support)
 *
 */
...
#if defined(CONFIG_CPU_SUBTYPE_SH7751)
static int NUM_WAYS = 1;
#else
#define NUM_WAYS 1 /* help compiler to optimise it out */
#endif
...
detect_cpu_and_cache_system(void)
{
	...
	unsigned long reg, id;

	/* we rely on being called before the PCI subsystem starts up */
...
void __init cache_init(void)
{
	...
	if (ccr & CCR_CACHE_ENABLE) {
		...
		for (addr = CACHE_OC_ADDRESS_ARRAY;
			...
			data = ctrl_inl(addr | (way << 14));
			if ((data & (CACHE_UPDATED|CACHE_VALID))
			    == (CACHE_UPDATED|CACHE_VALID))
				ctrl_outl(data & ~CACHE_UPDATED, addr | (way << 14));
		}
	    }
	}

	ctrl_outl(CCR_CACHE_INIT, CCR); /* SH4 emode */
...
void flush_cache_sigtramp(unsigned long addr)
{
	...
	if (NUM_WAYS > 1)
		ctrl_outl(0, index | (1 << 13));	/* Clear out Valid-bit */
	back_to_P1();
	restore_flags(flags);
}
 ...
 */
void __flush_dcache_page(unsigned long phys)
{
	unsigned long addr, data, way;
	unsigned long flags;
	...
	for (addr = CACHE_OC_ADDRESS_ARRAY;
	     addr < (CACHE_OC_ADDRESS_ARRAY
		     +(CACHE_OC_NUM_ENTRIES<< CACHE_OC_ENTRY_SHIFT));
	     addr += (1<<CACHE_OC_ENTRY_SHIFT)) {
		data = ctrl_inl(addr | (way << 14))&(0x1ffff000|CACHE_VALID);
		if (data == phys)
			ctrl_outl(0, addr | (way << 14));
	}
    }
	...
	for (addr = CACHE_IC_ADDRESS_ARRAY;
		...
		data = ctrl_inl(addr | (way << 13))&(0x1ffff000|CACHE_VALID);
		if (data == phys) {
			printk(KERN_INFO "__flush_cache_page: I-cache entry found\n");
			ctrl_outl(0, addr | (way << 13));
		}
	}
    }
#endif
...
void flush_cache_all(void)
{
	...
	for (addr = (unsigned long)empty_zero_page;
	     addr < (unsigned long)empty_zero_page + 1024*16*NUM_WAYS;
	     addr += L1_CACHE_BYTES)
...
void flush_cache_page(struct vm_area_struct *vma, unsigned long address)
{
	...
	pte_t *pte;
	pte_t entry;
	unsigned long phys, addr, data, way;
	unsigned long flags;
