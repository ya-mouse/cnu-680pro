 ...
 * ***  N O T E  ***
 *
 * Please do not fsck with this file.  Instead, if you want to make changes
 * send them by email to rmk@arm.linux.org.uk.  I do not want to fix this
 * file in the middle of a stable kernel series.
 */
...
//ryc++
static inline long read_roh(struct file *file, char *buf, off_t count)
{
	extern long sys_read_roh(struct file *, char *, int);
	return sys_read_roh(file, buf, count);
}
