/*
 *  linux/include/asm-armnommu/io.h
 *
...
/*
 * IO definitions.  We define {out,in,outs,ins}[bwl] if __io is defined by
...
#define outb(v,p)			__raw_writeb(v,__iob(p))
#define outw(v,p)			__raw_writew(v,__io(p))
#define outl(v,p)			__raw_writel(v,__io(p))

#define inb(p)				__raw_readb(__iob(p))
#define inw(p)				__raw_readw(__io(p))
#define inl(p)				__raw_readl(__io(p))

#define outsb(p,d,l)			__raw_writesb(__iob(p),d,l)
#define outsw(p,d,l)			__raw_writesw(__io(p),d,l)
#define outsl(p,d,l)			__raw_writesl(__io(p),d,l)

#define insb(p,d,l)			__raw_readsb(__iob(p),d,l)
...
//ryc++
#define memcpy_to_s3c2510_io(a,b,c)		_memcpy_to_s3c2510_io(__mem_pci(a),(b),(c))
//#define memcpy_to_s3c2510_io(a,b,c)		_memcpy_to_s3c2510_io((u8)(a),(b),(c))
