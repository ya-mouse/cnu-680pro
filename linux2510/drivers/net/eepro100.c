...
static int txfifo = 15;		/* Tx FIFO threshold in 4 byte units, 0-15 */
...
#ifdef PCI_DEBUG
static int pc_debug = PCI_DEBUG;
#define DEBUG(n, format, args...) \
do { \
    if (pc_debug>(n)) \
    {   char __buf[100]; \
        sprintf(__buf, format ,##args); \
        printk("%s[%d], %s(): %s", __FILE__, __LINE__, __FUNCTION__, __buf ); \
    }\
}while(0)
#else
#define DEBUG(n, args...)
...
static int speedo_debug = 3; //ryc++ for debugging
//static int speedo_debug = 6; //ryc--
...
static inline void wait_for_cmd_done(long cmd_ioaddr)
{
	...
	if (wait < 0)
		printk("eepro100: wait_for_cmd_done timeout!\n");
#endif
}
		...
		const struct pci_device_id *ent)
{
	...
	if (!request_region(pci_resource_start(pdev, 1),
		...
		printk ("eepro100: cannot reserve I/O ports\n");
		goto err_out_none;
	}
// Radicalis_hans_begin (03.12.27) - for changed resource mapping
//	if (!request_mem_pci_region(pci_resource_start(pdev, 0),
	if (!request_mem_region(pci_resource_start(pdev, 0),
// Radicalis_hans_end (03.12.27) - for changed resource mapping
			pci_resource_len(pdev, 0), "eepro100")) {
		printk ("eepro100: cannot reserve MMIO region\n");
	...
	if (!ioaddr) {
		printk ("eepro100: cannot remap MMIO region %lx @ %lx\n",
	...
	if (pci_enable_device(pdev)){
		printk(" goto err_out_free_mmio_region\n");
		goto err_out_free_mmio_region;
	}
	pci_set_master(pdev);
...
// Radicalis_hans_begin (03.12.27) - for changed resource mapping
//	release_mem_pci_region(pci_resource_start(pdev, 0), pci_resource_len(pdev, 0));
		...
		long ioaddr, int card_idx, int acpi_idle_state)
{
	...
	DEBUG(9, "tx_ring_space=0x%08x., tx_ring_dma=0x%08x.\n", tx_ring_space, tx_ring_dma);
	...
	if (dev == NULL) {
		printk("eepro100: Could not allocate ethernet device.\n");
	...
	*/
	{
		...
		/* Use IO only to avoid postponed writes and satisfy EEPROM timing
		   requirements. */
		iobase = pci_resource_start(pdev, 1); //ryc++
		...
		if (sum != 0xBABA)
			printk("%s: Invalid EEPROM checksum %#4.4x, "
	   ...
	   them for bug reports. */
	{
		...
		if ((eeprom[3] & 0x03) != 0x03)
			printk("  Receiver lock-up bug exists -- enabling"
				   " work-around.\n");
		printk("  Board assembly %4.4x%2.2x-%3.3d, Physical"
			   " connectors present:",
		...
		if (eeprom[7] & 0x0700)
			printk("    Secondary interface chip %s.\n",
				   phys[(eeprom[7]>>8)&7]);
		...
		if ((option >= 0) && (option & 0x70)) {
			printk("  Forcing %dMbs %s-duplex operation.\n",
				   (option & 0x20 ? 100 : 10),
		...
		if (boguscnt < 0) {		// Test optimized out. 
			DEBUG(4, "Self test failed, status %8.8x:\n"
				   KERN_ERR " Failure to initialize the i82557.\n"
				   KERN_ERR " Verify that the card is a bus-master"
				   " capable slot.\n",
				   self_test_results[1]);
		} else
			printk("  General self-test: %s.\n"
	...
	if (sp->rx_bug)
		printk("  Receiver lock-up workaround activated.\n");
...
static int mdio_read(long ioaddr, int phy_id, int location)
{
	...
	do {
		...
		if (--boguscnt < 0) {
			printk(" mdio_read() timed out with val = %8.8x.\n", val);
			break;
		}
...
static int mdio_write(long ioaddr, int phy_id, int location, int value)
{
	...
	do {
		val = inl(ioaddr + SCBCtrlMDI);
		if (--boguscnt < 0) {
			printk(" mdio_write() timed out with val = %8.8x.\n", val);
			break;
		}
	} while (! (val & 0x10000000));
	return val & 0xffff;
...
speedo_open(struct net_device *dev)
{
	...
	if (speedo_debug > 1)
		printk("%s: speedo_open() irq %d.\n", dev->name, dev->irq);

	MOD_INC_USE_COUNT;

#ifndef CONFIG_NOWRITEBACK
	tempdmabuffer =  kmalloc(PKT_BUF_SZ, GFP_ATOMIC | GFP_DMA);
#endif
	...
	if (speedo_debug > 2) {
		printk("%s: Done speedo_open(), status %8.8x.\n",
...
static void speedo_resume(struct net_device *dev)
{
	...
	//sp->tx_threshold = 0x01208000; // original code
	/* ryc adjusted this value because of tx underrun symptom */
	sp->tx_threshold = 0x01808000;
	...
	if (sp->rx_ringp[sp->cur_rx % RX_RING_SIZE] == NULL) {
		if (speedo_debug > 2)
			printk("%s: NULL cur_rx in speedo_resume().\n",
					dev->name);
	} else {
		wait_for_cmd_done(ioaddr + SCBCmd);
...
static void speedo_timer(unsigned long data)
{
	...
	if ((sp->phy[0] & 0x8000) == 0) {
		...
		if (partner != sp->partner) {
			...
			if (speedo_debug > 2) {
				...
				printk("%s: Old partner %x, new %x, adv %x.\n",
	...
	if (speedo_debug > 3) {
		printk("%s: Media control tick, status %4.4x.\n",
			   dev->name, inw(ioaddr + SCBStatus));
	}
	if (sp->rx_mode < 0  ||
		...
		if (speedo_debug > 3)
			printk("%s: Sending a multicast list set command"
...
static void speedo_show_state(struct net_device *dev)
{
	...
	if (speedo_debug > 0) {
		int i;
		printk("%s: Tx ring dump,  Tx queue %u / %u:\n", dev->name,
			   sp->cur_tx, sp->dirty_tx);
		for (i = 0; i < TX_RING_SIZE; i++)
			printk("%s:  %c%c%2d %8.8x.\n", dev->name,
				   i == sp->dirty_tx % TX_RING_SIZE ? '*' : ' ',
				   i == sp->cur_tx % TX_RING_SIZE ? '=' : ' ',
				   i, sp->tx_ring[i].status);
	}
	printk("%s: Printing Rx ring"
		   " (next to receive into %u, dirty index %u).\n",
		   dev->name, sp->cur_rx, sp->dirty_rx);

	for (i = 0; i < RX_RING_SIZE; i++)
		printk("%s: %c%c%c%2d %8.8x.\n", dev->name,
...
speedo_init_rx_ring(struct net_device *dev)
{
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		struct sk_buff *skb;
		//skb = dev_alloc_skb(PKT_BUF_SZ + sizeof(struct RxFD)); //ryc--

		/* ryc++ modified for 2510 */
		skb = __dev_alloc_skb(PKT_BUF_SZ + sizeof(struct RxFD),
						                    GFP_ATOMIC | GFP_DMA);
		sp->rx_skbuff[i] = skb;
...
speedo_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if ((int)(sp->cur_tx - sp->dirty_tx) >= TX_QUEUE_LIMIT) {
		printk("%s: incorrect tbusy state, fixed.\n", dev->name);
	...
	sp->tx_ring[entry].tx_buf_addr0 = cpu_to_le32(skb->data);
#else
	if(skb->data >= DMA_START)
	{
		sp->tx_ring[entry].tx_buf_addr0 = cpu_to_le32(skb->data);
		DEBUG(9, "sp->tx_ring[entry].tx_buf_addr0 = 0x%lx.\n", sp->tx_ring[entry].tx_buf_addr0);
	}
	else
	{
		memcpy( tempdmabuffer, skb->data, skb->len);
		sp->tx_ring[entry].tx_buf_addr0= tempdmabuffer;
	}
#endif
	...
	if ((int)(sp->cur_tx - sp->dirty_tx) >= TX_QUEUE_LIMIT) {
		DEBUG(4, " space size = %x, q_limit = %x\n",(sp->cur_tx - sp->dirty_tx),
						TX_QUEUE_LIMIT);
		DEBUG(4, "tx full\n");
		netif_stop_queue(dev);
		sp->tx_full = 1;
...
static void speedo_tx_buffer_gc(struct net_device *dev)
{
	...
	while ((int)(sp->cur_tx - dirty_tx) > 0) {
		...
		if (speedo_debug > 5)
			printk(" scavenge candidate %d status %4.4x.\n",
				   entry, status);
		...
		if (status & TxUnderrun)
			if (sp->tx_threshold < 0x01e08000) {
				if (speedo_debug > 2)
					printk("%s: TX underrun, threshold adjusted.\n",
						   dev->name);
...
static void speedo_interrupt(int irq, void *dev_instance, struct pt_regs *regs)
{
...
#ifndef final_version
	if (dev == NULL) {
		printk("speedo_interrupt(): irq %d for unknown device.\n", irq);
		return;
	}
#endif
	...
	if (test_and_set_bit(0, (void*)&sp->in_interrupt)) {
		printk("%s: SMP simultaneous entry of an interrupt handler.\n",
			   dev->name);
	...
	do {
		...
		if ((sp->rx_ring_state&(RrNoMem|RrNoResources)) == RrNoResources) {
			printk("%s: restart the receiver after a possible hang.\n",dev->name);
			spin_lock(&sp->lock);
			/* Restart the receiver.
	...
	if (sp->rx_skbuff[entry] == NULL) {
		...
		if (rxf == NULL) {
			...
			if (speedo_debug > 2 || !(sp->rx_ring_state & RrOOMReported)) {
				printk("%s: can't fill rx buffer (force %d)!\n",
						dev->name, force);
				speedo_show_state(dev);
...
speedo_rx(struct net_device *dev)
{
	...
	while (sp->rx_ringp[entry] != NULL) {
		...
		if (speedo_debug > 4)
			printk("  speedo_rx() status %8.8x len %d.\n", status,
				   pkt_len);
		if ((status & (RxErrTooBig|RxOK|0x0f90)) != RxOK) {
			if (status & RxErrTooBig)
				printk("%s: Ethernet frame overran the Rx buffer, "
					   "status %8.8x!\n", dev->name, status);
			else if (! (status & RxOK)) {
				/* There was a fatal error.  This *should* be impossible. */
				sp->stats.rx_errors++;
				printk("%s: Anomalous event in speedo_rx(), "
					   "status %8.8x.\n",
					   dev->name, status);
			}
		} else {
			...
			if (pkt_len < rx_copybreak
				&& (skb = __dev_alloc_skb((pkt_len + 2),(GFP_ATOMIC| GFP_DMA))) != 0) {
				skb->dev = dev;
				...
				if (skb == NULL) {
					printk("%s: Inconsistent Rx descriptor chain.\n",
						   dev->name);
					break;
				}
...
*/
	...
	sp->mc_setup_tail = NULL;
	if (speedo_debug > 0)
		printk("%s: %d multicast blocks dropped.\n", dev->name, i);
...
static void set_rx_mode(struct net_device *dev)
{
	...
	if (new_rx_mode == 0  &&  dev->mc_count < 4) {
		...
		*setup_params++ = cpu_to_le16(dev->mc_count*6);
		...
		mc_blk->frame_dma = &mc_blk->frame;
		mc_setup_frm = &mc_blk->frame;

		/* Fill the setup frame. */
		if (speedo_debug > 1)
			printk("%s: Constructing a setup frame at %p.\n",
...
static int pci_module_init(struct pci_driver *pdev)
{
	...
	if (rc <= 0) {
		printk("%s: No cards found, driver not installed.\n",
			   pdev->name);
...
static int __init eepro100_init_module(void)
{
	...
	if (debug >= 0 && speedo_debug != debug)
		printk("eepro100.c: Debug level is %d.\n", debug);
/*	if (debug >= 0)
		speedo_debug = debug; */
#endif
