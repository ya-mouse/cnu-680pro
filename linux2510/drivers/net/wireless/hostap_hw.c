...
static int channel[MAX_PARM_DEVICES] = { 1, DEF_INTS };
MODULE_PARM(channel, PARM_MIN_MAX "i");
MODULE_PARM_DESC(channel, "Initial channel");

static char *essid[MAX_PARM_DEVICES] = { "WLAN_AP" };
		       ...
		       u16 *param1, u16 *resp0)
{
	...
	if (in_interrupt()) {
		printk("%s: hfa384x_cmd called from interrupt "
		       "context\n", dev->name);
		return -1;
	}

	if (local->cmd_queue_len >= HOSTAP_CMD_QUEUE_MAX_LEN) {
		printk("%s: hfa384x_cmd: cmd_queue full\n",
		       dev->name);
		return -1;
	}
	...
	if (entry == NULL) {
		printk("%s: hfa384x_cmd - kmalloc failed\n",
		       dev->name);
		return -ENOMEM;
	}
	...
	if (!err) {
		if (hfa384x_cmd_issue(dev, entry)){
			printk("hfa384x_cmd_issue ERROR\n"); //ryc++
			err = -ETIMEDOUT;
		}
		else
			issued = 1;
 ...
 wait_completion:
	...
	spin_lock_irqsave(&local->cmdlock, flags);
	if (!list_empty(&entry->list)) {
		printk("%s: hfa384x_cmd: entry still in list? "
	...
	if (err) {
		printk("%s: hfa384x_cmd: interrupted; err=%d\n",
		       dev->name, err);
		res = err;
		goto done;
	}
	...
	if (entry->type != CMD_COMPLETED) {
		...
		printk("%s: hfa384x_cmd: command was not "
			   ...
			   int exact_len)
{
	...
	if (res) {
		printk("%s: hfa384x_get_rid: CMDCODE_ACCESS failed "
		       "(res=%d, rid=%04x, len=%d)\n",
...
static int prism2_hw_init(struct net_device *dev, int initial)
{
 ...
 init:
	...
	printk("prism2_hw_init: initialized in %d iterations\n",
	       HFA384X_INIT_TIMEOUT - i);
	HFA384X_OUTW(HFA384X_EV_CMD, HFA384X_EVACK_OFF);
	
	/* ryc++ for reset problem - the following two lines*/
	printk("roh INT enabled\n");
	rPCIINTEN = 0xffffffff; //all enable for viewing status. 20031113  for hw reset
	...
	for (;;) {
		...
		if (ev == 0xffff) {
			if (local->shutdown)
				return IRQ_HANDLED;
			HFA384X_OUTW(0xffff, HFA384X_EVACK_OFF);
			printk("%s: prism2_interrupt: ev=0xffff\n",
			       dev->name);
			PRISM2_SCHEDULE_TASK(&local->reset_queue);
			return IRQ_HANDLED;
		}
