/*======================================================================
    Modified from integrator-flash.c
    spark 20031013
======================================================================*/
/*======================================================================
   ...
   $Id: integrator-flash.c,v 1.1.1.1 2003/11/17 02:34:02 jipark Exp $
...
static struct mtd_partition sc2510_partitions[] = {
        /* Reserved for boot code, read-only */
        {
                name: "flash_boot",
                offset: 0x00000000,
                size: CONFIG_MTD_SC2510_RESERVE,
                // by jkglee, 2002.11.25 to make this area writeable ...
...
#if 0
        {
                ...
                size: CONFIG_MTD_SC2510_DEFAULT_MAC_SIZE,
        },
#endif
        {
                name: "data_setting",
                offset: MTDPART_OFS_APPEND,
                size: CONFIG_MTD_SC2510_DATA_SETTING_SIZE,
        },
        {
                name: "kernel",
                offset: MTDPART_OFS_APPEND,
                size: CONFIG_MTD_SC2510_KERNEL_SIZE,
                /*spark 20031120*/
...
#define FLASH_BASE              FLASH_MEM_BASE
...
static int __init armflash_cfi_init(void *base, u_int size)
{
	...
	armflash_map.size       = size;
        armflash_map.buswidth   = 2; /*spark 20031013----4; */
	...
	if (!mtd) {
	   mtd = do_map_probe("jedec_probe", &armflash_map);
	   if (!mtd) {
		printk("mtd probe flash failed\n");   
		return -ENXIO;
	   }	
        } 
	...
	mtd->module = THIS_MODULE;
        ...
        //ret = parse_afs_partitions(mtd, sc2510_partitions);

        //if (ret > 0) {
                ret = add_mtd_partitions(mtd, sc2510_partitions, 4);/*spark 20031112*/
                /* we don't need the partition info any longer */
                //kfree(parts);
                if (ret)
                        printk("mtd partition registration "
                                "failed: %d\n", ret);
        //}
        /*
