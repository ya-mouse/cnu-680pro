...
#define  ENABLE_WDT     1       /* for watch dog */
...
#ifdef ENABLE_WDT
static int delay_WDS_reset_time = 6000; /* 6000 * 40ms = 240 sec */ 
#endif
...
{
    unsigned int *pnWDTREG = (unsigned int *)0xF0040008;
    printk("init wdt mode\n");
    *pnWDTREG = 0;
}
...
******************************************************
*/
static void update_wdt()
{
    unsigned int *pnWDTREG = (unsigned int *)0xF0040008;

    if (delay_WDS_reset_time > 0) {       
        delay_WDS_reset_time--;
    }
    if (delay_WDS_reset_time==1) {
       printk("start WDS timer\n");
    } 
    if (delay_WDS_reset_time <=0) {
        *pnWDTREG = 0xFFF20000; /* refer to datasheet page 638, will do reset after 8 sec */
    }
}
#endif
...
#if ENABLE_WDT
    /* by coreyliu, 20040518 add watch dog */
    init_wdt();
#endif
