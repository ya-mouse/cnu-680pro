 ...
 * 2002-10-20: modified pbus_assign_resources_sorted() for S3C2510
 * 			   by Roh you-chang(terius90@samsung.com)
 * 2003-05-22: inserted  pci_bridge_check_io() for compilation at uClinux
 *             2.4.20 distribution version by Roh you-chang(terius90@samsung.com)
 */
	...
	for (ln=bus->devices.next; ln != &bus->devices; ln=ln->next) {
		...
		if (class == PCI_CLASS_DISPLAY_VGA
			...
			cmd &= ~(PCI_COMMAND_IO | PCI_COMMAND_MEMORY
						| PCI_COMMAND_MASTER);
			*/
			/* ryc modified the following line to avoid kernel panic!!! */
            if(dev->device == 0xa510){ /* when device is 2510X host device */
                cmd &= ~(PCI_COMMAND_IO | PCI_COMMAND_MEMORY);
            }else{
                cmd &= ~(PCI_COMMAND_IO | PCI_COMMAND_MEMORY
                        | PCI_COMMAND_MASTER);
...
// Radicalis_hans_begin (03.12.27) - avoid to assign resource for s3c2510
            if(dev->device != 0xa510) /* when device is not 2510X host device */
// Radicalis_hans_begin (03.12.27) - avoid to assign resource for s3c2510
