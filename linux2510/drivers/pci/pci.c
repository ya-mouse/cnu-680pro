/*
 *	$Id: pci.c,v 1.2 2003/12/30 22:41:41 kimhans Exp $
 *
 ...
 *	history:
 *		2002-10-14: modified pci_read_irq() for S3C2510 
 *					by roh you-chang(terius90@samsung.com)
 *		2002-11:10: modified pci_alloc_primary_bus() for S3C2510
 *					by roh you-chang(terius90@samsung.com)
 *		2003-05-23: modified pci_match_device() for PCI_ANY devices
 *					by roh you-chang(terius90@samsung.com)
 */
...
// Radicalis_hans_begin (2003.12.24) - to original code
//		release_mem_pci_region(pci_resource_start(pdev, bar), /*ryc modified */
		release_mem_region(pci_resource_start(pdev, bar),
// Radicalis_hans_end (2003.12.24)
				pci_resource_len(pdev, bar));
}
...
int pci_request_region(struct pci_dev *pdev, int bar, char *res_name)
{
	...
	if (pci_resource_flags(pdev, bar) & IORESOURCE_IO) {
		...
		/* ryc modified for S3C2510 */
// Radicalis_hans_begin (2003.12.24) - to original code
//		if (!request_mem_pci_region(pci_resource_start(pdev, bar),
...
err_out:
	printk ("PCI: Unable to reserve %s region #%d:%lx@%lx for device %s\n",
...
{
	unsigned int s3c2510_device_id = 0xa510;
	while (ids->vendor || ids->subvendor || ids->class_mask) {
/* ryc ++ for s3c2510 
 *    when checking device_id, the id of 2510 is host. So you don't need check 
 *    device_id of s3c2510
 */
		if(dev->device == s3c2510_device_id)
			return NULL;
...
static void pci_read_irq(struct pci_dev *dev)
{
	...
	if (irq){
		pci_read_config_byte(dev, PCI_INTERRUPT_LINE, &irq);
		 /* ryc++ - interrupt line number must be 15 */
        pci_write_config_byte(dev, PCI_INTERRUPT_LINE, 0xf); //ryc++
	}
	dev->irq = irq;
}
...
unsigned int __devinit pci_do_scan_bus(struct pci_bus *bus)
{
	...
	struct pci_dev *dev, dev0;
        //spark 20031223 check the ralink card is inserted
        int found_ralink=0;
        struct pci_dev *ptmpdev;
	...
	/* Go find them, Rover! */
#ifndef CONFIG_S3C2510_CARDBUS
    for (devfn = 0; devfn < 0x100; devfn += 8) {
        dev0.devfn = devfn;
        //spark 20031223 check the ralink card is inserted	
        ptmpdev=pci_scan_slot(&dev0);
//      printk("pci.c: pci_scan_bus:%dvendorID=%X\n",devfn,ptmpdev->vendor);
        if(ptmpdev->vendor==0x1814)
	found_ralink=1;
    }
#else
    for (devfn = 0; devfn < 0x10; devfn += 8) {
        ...
        if(!found_ralink){
            //spark 20031219
            printk("pci.c: pci_scan_bus:call led ioctl ERROR_LED\n");
            gpio_kernel_ioctl(GPIO_LED_CMD,GPIO_WRITE_ERROR_LED_ON);
            gpio_kernel_ioctl(GPIO_LED_CMD,GPIO_WRITE_WIRELESS_LED_OFF);
	}
...
struct pci_bus * __devinit pci_alloc_primary_bus(int bus)
{
	...
	b->resource[0] = &ioport_resource;
	 /* ryc++ we should make mapping to ioxxx_pci_resource because I already
     *       setup at setup_resource routine.
     */
// Radicalis_hans_begin (2003.12.24) - to original code
//	b->resource[1] = &iomem_pci_resource; //ryc++ for S3C2510
