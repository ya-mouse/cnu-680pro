 ...
 *
 *	Version:	$Id: skbuff.c,v 1.2 2003/12/24 07:29:44 odorong Exp $
 *
 *	Fixes:	
...
// 8MSol_jungmin_begin (03.12.24)
#if 	1 	/* parkjy. */
			{
				...
				if((unsigned int)page %4 != 0) {
					static int cnt = 0;
					printk("unaligned page at skb_release data.
						dev %s skb=0x%x, skb->head=%x, page=%x cnt=%d\n", 
						skb->dev->name, skb, skb->head, page, cnt++);
				}
#endif
