...
LIST_HEAD(protocol_list);       // tshwang 06/01/2005 for throughput
#endif
static LIST_HEAD(helpers);
/* 8MSol_choish , modified for NAPI */
#ifdef CONFIG_S3C2510_NAPI 
unsigned int ip_conntrack_htable_bits;
static int ip_conntrack_vmalloc;
struct ip_conntrack_protocol *ip_ct_protos[MAX_IP_CT_PROTO];    // tshwang 06/01/2005 for throughput
#else
...
static void rehash_conntracks(void)
{
		...
		/* Double timeout if we can: no point furiously rehashing */
...
#ifndef CONFIG_S3C2510_NAPI     // tshwang 06/01/2005 for throughput
...
#endif      // tshwang 06/01/2005 for throughput
...
/* 8MSol_choish , replace hash_conntrack func for NAPI */
#ifndef CONFIG_S3C2510_NAPI
...
hash_conntrack(const struct ip_conntrack_tuple *tuple)
{
		...
		hash ^= (tuple->src.u.all + ((u32)tuple->dst.u.all << 16));
		hash ^= tuple->dst.ip;
		hash ^= ((tuple->src.ip << 13) | (tuple->src.ip >> 19));
...
/* 8MSol_choish, replace conntrack_tuple_cmp func for NAPI */
#ifndef CONFIG_S3C2510_NAPI
static inline int
...
/* 8MSol_choish, replace ip_conntrack_tuple_hash func for NAPI */
#ifndef CONFIG_S3C2510_NAPI
...
//	unsigned int hash, repl_hash;
	unsigned int hash, repl_hash, count; /* 8MSol_choish, for NAPI */
	struct ip_conntrack *ct;
	...
	if (!LIST_FIND(&ip_conntrack_hash[hash],
		       conntrack_tuple_cmp,
		       struct ip_conntrack_tuple_hash *,
				&ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple, NULL ,&count) /* 8MSol_choish, for NAPI */
//				&ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple, NULL)
				&& !LIST_FIND(&ip_conntrack_hash[repl_hash],
			  conntrack_tuple_cmp,
			  struct ip_conntrack_tuple_hash *,
			  &ct->tuplehash[IP_CT_DIR_REPLY].tuple, NULL, &count)) { /* 8MSol_choish, for NAPI */
	       ...
	       struct sk_buff *skb)
{
	...
	int i;
	static unsigned int drop_next = 0; // 8MSol_choish

	hash = hash_conntrack(tuple);

/* 8MSol_choish , replace for NAPI */
#ifndef CONFIG_S3C2510_NAPI
	if (ip_conntrack_max &&
    ...
    if(ip_conntrack_max
		&& atomic_read(&ip_conntrack_count) >= ip_conntrack_max) {
			...
			if (net_ratelimit())
/*
   8MSol2510_jungmin (03.11.20)
   base on ww_le_v2.0c

   When P2P program search too many IP, Connection table will be overflowed
   Now printk will be remarked as wuluv suggest
				printk(KERN_WARNING
...
#else
		if (ABS(jiffies + extra_jiffies - ct->timeout.expires) >= HZ 
...
#ifndef CONFIG_S3C2510_NAPI  // 8MSol_choish , replace for NAPI
			ip_conntrack_htable_size = 8192;
		if (ip_conntrack_htable_size < 16)
			ip_conntrack_htable_size = 16;
	}

        /* by jkglee, 2003.03.21, extend sesson num to 4096*/
...
/* 8MSol_choish, replace for NAPI */
    ...
    if(!ip_conntrack_hash) {
        ...
        printk("ip_conntrack: falling back to vmalloc.\n");
        ip_conntrack_hash=vmalloc(sizeof(struct list_head)
                         * htable_size());
    }
#endif	
...
/* 8MSol_choish, replace for NAPI */
	...
	for (i = 0; i < ip_conntrack_htable_size; i++)
#else
    // tshwang 06/01/2005 for throughput
    /* Don't NEED lock here, but good form anyway. */
    WRITE_LOCK(&ip_conntrack_lock);
