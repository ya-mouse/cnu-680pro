...
LIST_HEAD(protos);  // tshwang 06/01/2005 for throughput
LIST_HEAD(helpers);
#else
struct ip_nat_protocol *ip_nat_protos[MAX_IP_NAT_PROTO];    // tshwang 06/01/2005 for throughput

extern unsigned int ip_conntrack_htable_bits;
...
#ifndef CONFIG_S3C2510_NAPI // 8MSol_choish 
static inline int
...
int __init ip_nat_init(void)
{
...
#else
	ip_nat_htable_size = htable_size();
#endif
	...
	for (i = 0; i < MAX_IP_NAT_PROTO; i++)
		ip_nat_protos[i] = &unknown_nat_protocol;
