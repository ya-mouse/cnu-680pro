...
#if 1
        /* by jkglee, 2002.11.18, for iptables support following sytle ...
	 * * iptables -A INPUT -s 192.168.1.1-192.168.1.100 -j ACCEPTi */
        if ((!(ipinfo->src_range_style) && FWINV((ip->saddr&ipinfo->smsk.s_addr)
                   != ipinfo->src.s_addr, IPT_INV_SRCIP) ) ||
           ( !(ipinfo->dst_range_style) && FWINV((ip->daddr&ipinfo->dmsk.s_addr)
     		   != ipinfo->dst.s_addr, IPT_INV_DSTIP) ) ||
           ( (ipinfo->src_range_style) && ((ntohl(ip->saddr)
                   < ntohl(ipinfo->src.s_addr)) ||
           (ntohl(ip->saddr) > ntohl(ipinfo->src2.s_addr))) ) ||
           ( (ipinfo->dst_range_style) && ((ntohl(ip->daddr)			                                                  < ntohl(ipinfo->dst.s_addr)) ||
           (ntohl(ip->daddr) > ntohl(ipinfo->dst2.s_addr))) ) )  {
#else		
