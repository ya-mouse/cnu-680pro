...
int has_expired(struct net_bridge *br,
	struct net_bridge_fdb_entry *fdb);
int br_mac_hash(unsigned char *mac);
extern int raw_net_send_packet(struct sk_buff *skb);       

extern int BRG_ENABLE_FLAG;
#endif                    
...
// 8MSol_jungmin_begin (03.12.23)
//wjluv add for RFC2684/1483 Routed IP PDUs
static unsigned char routed_pad[] =
    { 0xAA, 0xAA, 0x03, 0x00, 0x00, 0x00, 0x08, 0x00 };

//wjluv comments
//pseudo mac is pseudo destination mac
//This mac should not be transmitted
static unsigned char pseudo_mac[6] =
    { 0x00,0x93,0x12,0x20,0x03,0x28 };

static unsigned char protocol_ip[2] =
    { 0x08,0x00};
// 8MSol_jungmin_end (03.12.23)
#define PADLEN	(2)
...
// 8MSol_jungmin_begin (03.12.23)
	...
	int minheadroom;
        //wjluv add for RFC2684/1483 Routed IP PDUs
	if((brvcc->modes == bridged)||(brvcc->modes == pppoe))
	{
	...
	else if (brvcc->modes == routed)
	{
		//wjluv comments
		//remove pseudo mac + source mac + protocol type
		//In user space,
		//arp -s <defaultgateway ip address> <pseudo mac> for preventing ARP query
		skb_pull(skb,14);
		minheadroom= (brvcc->encaps == e_llc) ? 8 : 2;
		if (skb_headroom(skb) < minheadroom) {
			struct sk_buff *skb2 = skb_realloc_headroom(skb, minheadroom);
			brvcc->copies_needed++;
			dev_kfree_skb(skb);
		...
		skb_push(skb, minheadroom);
		if (brvcc->encaps == e_llc)
			memcpy(skb->data, routed_pad, 8);
		else
			memset(skb->data, 0, 2);
	}
	/*else if (brvcc->modes == pppoe)*/
		// in PPPoE do nothing
	/*	;*/
#endif
// 8MSol_jungmin_end (03.12.23)	
...
// 8MSol_jungmin_begin (03.12.23)
extern int save_br_dev[8];
extern int br_dev_xmit_raw(struct sk_buff *skb, struct net_device *dev);
extern int __br_dev_xmit_raw(struct sk_buff *skb, struct net_device *dev);

#ifdef CONFIG_BRIDGE_TUNNEL
// return 1: deliver packets
//   0: packet will be blocked because the path is from nas_x to nas_y
unsigned int should_deliver_in_br2684(struct net_bridge_port *p, struct sk_buff *skb)
{
	struct list_head *lhd;
	struct br2684_dev *brdev;
	unsigned result  = 0;   

	// Check for net_bridge_port
	list_for_each(lhd, &br2684_devs){
		brdev = list_entry_brdev(lhd);
		if(p->dev == &brdev->net_dev){
			result =1;  // device is in br2684 bridge
			break;
		}	
	}
	if(result != 1) return 1;   // deliver packet, because 1 port is not in br2684 bridge.

	// Check for sk_buff
	list_for_each(lhd, &br2684_devs){
		brdev = list_entry_brdev(lhd);
		if(skb->dev == &brdev->net_dev){
			return 0;   // block this packet, both port is in br2684 bridge.
			break;
		}
	}
	return 1;
...
void add_new_fdb_in_br2684(struct net_bridge_fdb_entry *fdb)
{
	struct list_head *lhd;
	struct br2684_dev *brdev;
	spinlock_t  lock;
	...
	list_for_each(lhd, &br2684_devs){
		brdev = list_entry_brdev(lhd);
		if(fdb->dst->dev == &brdev->net_dev){
			spin_lock_irqsave(&lock, flags);
			brdev->tunnel_on = 1;   //20030814 for EOA
			brdev->fdb = fdb;
			spin_unlock_irqrestore(&lock, flags);
#if 0
			printk("add to fdb] dev %.16s: num=%d, mac=%02X:%02X:"
					"%02X:%02X:%02X:%02X (%s)\n", brdev->net_dev.name,
					brdev->number,
					brdev->net_dev.dev_addr[0], brdev->net_dev.dev_addr[1],
					brdev->net_dev.dev_addr[2], brdev->net_dev.dev_addr[3],
					brdev->net_dev.dev_addr[4], brdev->net_dev.dev_addr[5],
...
// delif =1 : interface is deleted from bridge
//       =0 : interface is not deleted from bridge 
void expire_fdb_in_br2684(struct net_bridge_fdb_entry *fdb, int delif)
{
	struct list_head *lhd;
	struct br2684_dev *brdev;
	...
	list_for_each(lhd, &br2684_devs){
		...
		if(fdb == brdev->fdb){
			if (delif == 1)                 //
		brdev->tunnel_on = 0;   //20030814 for EOA
			brdev->fdb = NULL;
#if 0
			printk("expired in fdb] dev %.16s: num=%d, mac=%02X:%02X:"
					"%02X:%02X:%02X:%02X (%s)\n", brdev->net_dev.name,
					brdev->number,
					brdev->net_dev.dev_addr[0], brdev->net_dev.dev_addr[1],
					brdev->net_dev.dev_addr[2], brdev->net_dev.dev_addr[3],
...
// else: netif_rx
//
static int check_bridge_learning(struct br2684_dev *brdev, struct sk_buff *skb)
{
	...
	if(brdev->tunnel_on == NULL)
	goto freeandout;        //20030814 for EOA

	h_source = skb->mac.ethernet->h_source;
...
#if 0  // spanning tree packets are multicase.
	if (br->stp_enabled &&
			...
			p->state == BR_STATE_FORWARDING)
	{
		...
		if(fdb == NULL || memcmp(fdb->addr.addr, h_source, ETH_ALEN))
		{   // different ethernet address, search new fdb for source
#if 0
			fdb = br->hash[br_mac_hash(h_source)];
			while (fdb != NULL) {
				if (!memcmp(fdb->addr.addr, h_source, ETH_ALEN)) {
					if (!has_expired(br, fdb)) {
						break;
					}
					fdb = NULL; // fdb entry is expired !!!
					break;
				}
				fdb = fdb->next_hash;
			}
			if(fdb == NULL)         // no entry or entry is expired !!!
			{
				goto freeandout;
			}
...
wegotfdb:
			if(fdb->dst != p) goto freeandout;
			if(has_expired(br, fdb))
			{
#if 0
				printk("expired, no NULL fdb] dev %.16s: num=%d, mac=%02X:%02X:"
						"%02X:%02X:%02X:%02X (%s)\n", brdev->net_dev.name,
						brdev->number,
						brdev->net_dev.dev_addr[0], brdev->net_dev.dev_addr[1],
						brdev->net_dev.dev_addr[2], brdev->net_dev.dev_addr[3],
			...
			// not expired, update aging time.
			fdb->ageing_timer = jiffies;
			//printk("ageing time extended\n");
	...
	if (dest[0] & 1) 
		goto freeandout;
	dst = br->hash[br_mac_hash(dest)];
	while (dst != NULL) {
		if (!memcmp(dst->addr.addr, dest, ETH_ALEN)) {
			if (!has_expired(br, dst)) {
				break;
			}
			dst = NULL;
			break;
...
#if 0   //hans - when only to ethernet 0
		skb_push(skb,14);
		raw_net_send_packet(skb);
#else   // - when generalized form
		skb_push(skb, 14);
		skb->dev = dst->dst->dev;
...
// 8MSol_jungmin_begin (03.12.23)
	...
	if((brvcc->modes == bridged)||(brvcc->modes == pppoe))
	{
		//24bytes for RFC2684 Bridged mode
		plen = sizeof(llc_oui_pid_pad) + ETH_HLEN;
		if (brvcc->encaps == e_llc) {
			/* let us waste some time for checking the encapsulation.
			   Note, that only 7 char is checked so frames with a valid FCS
			   are also accepted (but FCS is not checked of course) */
	...
	{
		//wjluv comments check RFC2684
		//8bytes for RFC2684 routed mode
		plen = sizeof(routed_pad);
		if (brvcc->encaps == e_llc) {
			/* let us waste some time for checking the encapsulation. */
			if (memcmp(skb->data, routed_pad, 8)) {
				brdev->stats.rx_errors++;
				dev_kfree_skb(skb);
				return;
			}
		} else {
			plen = PADLEN;	/* pad, dstmac,srcmac, ethtype */
			/* first 2 chars should be 0 */
			if (*((u16 *) (skb->data)) != 0) {
				brdev->stats.rx_errors++;
				dev_kfree_skb(skb);
...
// 8MSol_jungmin_being (03.12.23)
#if 0
	skb_pull(skb, plen - ETH_HLEN);
#else
	/*wbk printk("DEBUG___step0: 0x%x, %d, 0x%x", (int)skb->data, plen,(int)skb->len );*/

        //wjluv comments
	//In routed modes, make ethernet frame using pseudo mac
	if((brvcc->modes == bridged)||(brvcc->modes == pppoe))
	{
		//only remove RFC2684 Bridged header 10bytes
		skb_pull(skb, plen - ETH_HLEN);
	}
	else if (brvcc->modes == routed)
	{
		//remove routed_pad
		skb_pull(skb,plen);
		//make ethernet frame using pseudo mac
		skb_push(skb,ETH_HLEN);
		memcpy(skb->data, atmvcc->dev->esi, 6);
		memcpy(skb->data+6, pseudo_mac, 6);
		memcpy(skb->data+12, protocol_ip, 2);
	}
#endif
...
// 8MSol_jungmin_begin (03.12.23)
	...
	netif_rx(skb);
#else
  	if(brvcc->modes == bridged)
	{
#ifdef CONFIG_BRIDGE_TUNNEL
	    	if(BRG_ENABLE_FLAG)
			{
				// if check_bridge_learning returns 0, goto upper layer
				// else, packets are gone to tunnel
				if(!check_bridge_learning(brdev, skb))
				{
					netif_rx(skb);
				}
...
// 8MSol_jungmin_begin (03.12.23)
            //wjluv add for RFC2684/1483 Routed IP PDUs
	    (be.modes != BR2684_BRIDGE && be.modes != BR2684_ROUTE && be.modes != BR2684_PPP) ||
// 8MSol_jungmin_end (03.12.23)
	    be.min_size != 0) {
		err = -EINVAL;
		goto error;
...
#else
        //wjluv add for RFC2684/1483 Routed IP PDUs
	DPRINTK("br2684_regvcc vcc=%p, modes=%d, encaps=%d, brvcc=%p\n", atmvcc, be.modes,  be.encaps,
		brvcc);
	printk("br2684_regvcc vcc=%p, modes=%d, encaps=%d, brvcc=%p\n", atmvcc, be.modes,  be.encaps,
		brvcc);
#endif
// 8MSol_jungmin_end (03.12.23)
	...
	// 8MSol_jungmin_begin (03.12.23)
        //wjluv add for RFC2684/1483 Routed IP PDUs
	brvcc->modes = (enum br2684_modes) be.modes;
	// 8MSol_jungmin_end (03.12.23)
...
#else
				return sprintf(buf, "  vcc %d.%d.%d:modes= %s encaps=%s"
#endif
// 8MSol_jungmin_end (03.12.23)
#ifndef FASTER_VERSION
				    ", failed copies %u/%u"
...
// 8MSol_jungmin_begin (03.12.23)
                                    //wjluv add for RFC2684/1483 Routed IP PDUs
				    (brvcc->modes  == bridged) ? "BRIDGED" : "ROUTED",
// 8MSol_jungmin_end (03.12.23)
