 ...
 *
 *	$Id: br_stp_if.c,v 1.1.1.1 2003/11/17 02:35:49 jipark Exp $
 *
...
void br_stp_recalculate_bridge_id(struct net_bridge *br)
{
	...
	p = br->port_list;
/* by jkglee, 203.05.15, remove bridege recaulate mac addresss assigned to br0 */
#if 0   
	while (p != NULL) {
		if (addr == br_mac_zero ||
...
#else
        /* 
	** by jkglee, 2003.06.09, using the same implement with AP101B, 
	** to use the port 1 address, the bridge id will
	** changed only when you delete port 1 interface and add another interface,
	** since the algorithm will add the last to a empty port with a small number
	*/
      	//if (p != NULL) {
	//      if (addr == br_mac_zero)
	//              addr = p->dev->dev_addr;
