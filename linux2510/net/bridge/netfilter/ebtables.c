...
// 8MSol_jungmin_begin (03.12.23)
#ifdef CONFIG_BRIDGE_TUNNEL
	{
		...
		for(lh = ebt_tables.next, prefetch(lh->next); lh != &ebt_tables;
				lh = lh->next, prefetch(lh->next)){
			t = lh;
			i += t->private->nentries;
		}
		if( i > 0 )
		{
			if(BRG_ENABLE_FLAG !=0) printk("Bridge tunneling  OFF, no. ebt =%d\n", i);
			BRG_ENABLE_FLAG = 0;
		}
		else
		{
			if(BRG_ENABLE_FLAG !=1) printk("Bridge tunneling ON, no. ebt =%d\n", i);
			BRG_ENABLE_FLAG = 1;
		}
	}
#endif
